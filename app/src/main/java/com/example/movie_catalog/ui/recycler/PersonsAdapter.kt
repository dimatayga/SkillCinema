    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            R.layout.item_film_page_person -> PersonGridVH(
                ItemFilmPagePersonBinding.inflate(inflater, parent, false))
            //The markup for the vertical full list of persons
            R.layout.item_person_list -> PersonListVH(
                ItemPersonListBinding.inflate(inflater, parent, false))
...
//    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        val person = linkers.getOrNull(position)?.person ?: return

        when (holder) {
            is PersonListVH -> bindPersonListVH(holder, person)
            is PersonGridVH -> bindPersonGridVH(holder, person)
        }
    }

    private fun bindPersonListVH(holder: PersonListVH, person: Person) {
        holder.binding.apply {
            personName.text = person.nameRu ?: ""
            personJob.text = person.professionText
            root.setOnClickListener { onClick(person) }
            val animationCard = LoadImageURLShow()
            animationCard.setAnimation(poster, person.posterUrl, R.dimen.card_film_radius)
        }
    }

    private fun bindPersonGridVH(holder: PersonGridVH, person: Person) {
        holder.binding.apply {
            tvActor.text = person.nameRu ?: ""
            tvRole.text = if (whatRole == 1) person.description ?: "" else person.professionText ?: ""
            root.setOnClickListener { onClick(person) }
            val animationCard = LoadImageURLShow()
            animationCard.setAnimation(photo, person.posterUrl, R.dimen.card_people_radius)
        }
    }

    override fun getItemCount(): Int {
        return if (linkers.size > sizeGird - 1 && sizeGird != 0) sizeGird else linkers.size
    }
//    If the size of the list is not specified, then we use the markup for the vertical full list of persons,
//    if the size is set, then we use the markup to display a grid of persons
