...
class DataSourceAPI @Inject constructor(private val retrofitApi: KinopoiskAPI) {
    ...
    suspend fun getInfoFilmSeason(film: Film) {
        ...
        filmInfoSeasons.infoFilm =
            film.filmId?.let { retrofitApi.getFilmInfo(it, DataCentre.headers) }
        ...
        if (filmInfoSeasons.infoFilm!!.serial!!) {
            ...
            filmInfoSeasons.infoSeasons =
                film.filmId?.let { retrofitApi.getSeasons(it, DataCentre.headers) }
        }
    ...
    suspend fun getPremieres() {
        ...
        val response = retrofitApi.getPremieres(currentYear, currentMonth, DataCentre.headers)
        if (response.isSuccessful) {
            val premieres = response.body()!!
            ...
            Log.e("DataSourceAPI", "getPremieres() response error: ${response.errorBody()?.string()}")
    ...
    suspend fun getFilters(page: Int, kit: Kit): List<Linker> {
        ...
        return when (kit) {
            ...
            Kit.RANDOM2 -> DataCentre.addFilms(
                retrofitApi.getFilters(
                    page,
                    SortingField.RATING.toString(), TypeFilm.FILM.toString(), 1, 10,
                    1990, 2100, "", kit.countryID, kit.genreID, DataCentre.headers
            ...
            Kit.SEARCH -> DataCentre.addFilms(
                retrofitApi.getFilters(
                    page,
                    filter.sorting.toString(),
                    filter.typeFilm.toString(),
                    filter.rating.first.toInt(),
                    filter.rating.second.toInt(),
                    filter.year.first,
                    filter.year.second,
                    filter.keyWord,
                    filter.country.id,
            ...
            Kit.SERIALS -> DataCentre.addFilms(
                retrofitApi.getFilters(
                    page, SortingField.RATING.toString(),
                    TypeFilm.TV_SERIES.toString(), 1, 10, 1900, 2100, DataCentre.headers
                ), kit
            )
